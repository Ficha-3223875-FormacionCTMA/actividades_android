...
import com.luciana.miformacionctma.domain.validarFecha
...
fun PantallaCrearActividad(
    ...
    var prioridadNombre by rememberSaveable {
        mutableStateOf(Prioridad.MEDIA.name)
    }
    ...
    val progresoError = if (intentoGuardar) validarProgreso(progreso) else null

    val fechaError = if (intentoGuardar) validarFecha(fecha) else null
