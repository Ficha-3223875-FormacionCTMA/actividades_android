...
fun AppNavigation(
    ...
    val refreshState = viewModel.refreshState.collectAsStateWithLifecycle().value
    val evidenciasState = viewModel.evidencias.collectAsStateWithLifecycle().value
    ...
    NavHost(
        ...
        composable("lista") {
            PantallaActividades(
                ...
                onReintentarSincronizacion = viewModel::sincronizar,
        ...
        composable("detalle/{actividadId}") { backStackEntry ->
            ...
            PantallaDetalleActividad(
                actividad = actividad,
                evidencias = if (evidenciasState.actividadId == actividad?.id) evidenciasState.evidencias else emptyList(),
                mensajeEvidencia = evidenciasState.mensaje,
                onCargarEvidencias = viewModel::cargarEvidencias,
                onElegirEvidencia = viewModel::guardarEvidencia,
                onPrepararCamara = viewModel::prepararUriCaptura,
                onCancelarCamara = viewModel::cancelarCaptura,
                onSubirEvidencia = viewModel::subirEvidencia,
                onEliminarEvidencia = viewModel::eliminarEvidencia,
                onLimpiarMensajeEvidencia = viewModel::limpiarMensajeEvidencia,
